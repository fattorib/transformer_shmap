"""TPU Benchmark for shard_map tensor and data parallelism."""
import argparse
...
def parse():
    parser = argparse.ArgumentParser(description="tensor-parallelism benchmark on TPU")
...
if __name__ == "__main__":
    ...
    if platform == "tpu":
        ...
        raise ValueError(
            "Expected TPU environment, but found GPU environment. Run `benchmark_gpu.py` for GPU benchmarking."
        )
    ...
    if jax.process_index() == 0:
        print(
            f"Distributing model with {param_count} params across {args.mp} device(s)."
        )
    ...
    convert_batch = partial(convert_arr_sharding, mesh=mesh, batch_spec=batch_spec)
    ...
    print(
        f"Per-Host BS {batch_size} - accum steps {accum_steps} - Num Executions {num_iter}"
    )
    ...
    flops_per_iter = flops_per_fwdbwd * batch_size * jax.process_count()
